...
function WithdrawForm() {
  ...
  const [balance, setBalance] = useState(
    ctx.user ? ctx.user.balance : "User is Not Logged In"
  );
        ...
        return (
          <Form>
            ...
            <ErrorMessage
              component={"div"}
              name="amount"
              className="form-text ms-2 text-danger"
            ></ErrorMessage>
            <div className="form-text ms-2 text-danger">{errorMessage}</div>

            <input
              type="submit"
              disabled={!ctx.user || props.values.amount <= 0 || errorMessage}
        ...
        show ? (
          ...
          >
            {(props) => {
              ...
              if (ctx.user) {
                const newBalance =
                  Number(ctx.user.balance) - props.values.amount;
                if (newBalance >= 0) {
                  errorMessage = "";
                } else {
                  errorMessage = "Insufficient Funds";
              ...
              return (
                <Form>
                  ...
                  <ErrorMessage
                    ...
                    className="form-text ms-2 text-danger"
                  ></ErrorMessage>
                  <div className="form-text ms-2 text-danger">
                    {errorMessage}
                  </div>
                  ...
                  <input
                    type="submit"
                    disabled={
                      !ctx.user || props.values.amount <= 0 || errorMessage
                    }
                    className="btn btn-light mt-3"
                    value="Withdraw"
                  />
